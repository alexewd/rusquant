...
"getOptionChain.Forts" <-
  ...
  {
    forts.URL <-
      "http://rts.micex.ru/ru/derivatives/optionsdesk-csv.aspx?sub=&marg=1&c2=on&c4=on&c6=on&c7=on"
    ...
    calls <-
      apply(calls, 2, function(x)
        suppressWarnings(as.numeric(x)))
    puts <-
      apply(puts, 2, function(x)
        suppressWarnings(as.numeric(x)))
    colnames <-
      c("Strike", "Last", "Chg", "Bid", "Ask", "Vol", "OI", "IV")
