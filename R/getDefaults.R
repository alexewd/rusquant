"importDefaults" <-
  function(calling.fun = NULL) {
    ...
    passed.args <- names(as.list(match.call(
      ...
      call = as.call(sys.call(-1))
    )))[-1]
    formal.args <- names(formals(as.character(sys.call(-1))))
    default.args <-
      names(which(sapply(all.defaults, function(x)
        ! is.null(x)) == TRUE))
        ...
        if (arg %in% default.args) {
          if (typeof(all.defaults[arg][[1]]) == 'list') {
            ...
            if (length(all.defaults[arg][[1]]) == 1) {
              ...
              assign(arg, as.character(parse(text = all.defaults[arg][[1]])), envir =
                       envir)
            }
          }
          else {
...
"useDefaults" <-
  function(name)
  {
    ...
    if (!identical(grep('importDefaults', deparse(body(name))), integer(0)))
      invisible(return())
    cc <-
      paste('if(exists(".importDefaults")) .importDefaults(calling.fun="',
            name,
            '")',
            sep = '')
    if (deparse(body(name))[1] == "{") {
      new.body <- as.call(parse(text = c(deparse(body(
        name
      ))[1],
      cc,
      deparse(body(
      ...
      new.body <-
        as.call(parse(text = c("{", cc, deparse(body(
          name
        ))[1], "}")))[[1]]
    }
...
"usingDefaults" <-
  function(name)
  {
    if (length(name) == 1) {
      if (is.function(name))
        name <- deparse(substitute(name))
      try(if (is.function(eval(parse(text = name)))) {
        return(!identical(grep('importDefaults', deparse(body(
          name
        ))[2:3]), integer(0)))
      }, silent = TRUE)
      return(NA)
...
"useDefaults.original" <-
  function(name)
  {
    ...
    if (identical(grep("(\\.usingDefaults) | (importDefaults)",
                       deparse(body(get(name, envir = globalenv())))),
                  integer(0))) {
      ...
      if (pkg != ".GlobalEnv") {
        ...
        assign(name, get(simple.name, envir = globalenv()), envir = globalenv())
      }
      new.body <- as.call(parse(
        text =
          c(
            "{",
            ".usingDefaults <- TRUE",
  ...
  function(name)
  {
    if (!identical(grep('\\.usingDefaults', deparse(body(name))),
      ...
      if (exists(paste('.', name, '.CALL', sep = ''), env, inherits = FALSE)) {
        ...
        remove(list = paste('.', name, '.CALL', sep = ''),
               envir = env)
      } else {
  ...
  function (.name, ...)
  {
    ...
    for (arg in names(new.defaults)) {
      if (!is.na(pmatch(arg, names(avail.defaults)))) {
        ...
        warning(paste(
          sQuote(arg),
          "was not set, possibly not a formal arg for",
          sQuote(.name)
        ))
      }
    }
    ...
    all.and.matched <-
      all.and.matched[unique(names(all.and.matched))]
    if (length(all.and.matched) == 0) {
      ...
      eval(parse(
        text = paste(
          ...
          "=list(",
          paste(paste(
            names(all.and.matched),
            "=",
            lapply(all.and.matched,
                   function(x) {
                     if (is.character(x)) {
                       deparse(x)
                     }
...
"getDefaults" <-
  function(name = NULL, arg = NULL) {
    ...
    if (!is.null(name)) {
      ...
      all.Defaults <- as.character(sapply(
        all.options[grep('.Default$', all.options)],
        FUN = function(x) {
          gsub('.Default$', '', x)
        }
      ))
