...
"getSymbols.Poloniex" <-
  function #S3 function (Poloniex is a class of first argument)
(
  Symbols,
  env,
  return.class = 'xts',
  ...
  {
  importDefaults("getSymbols.Poloniex")
  #rewrite default values if specified by setDefaults
  ...
  Polo.from <-
    as.numeric(as.POSIXct(from)) #convert to UNIX timestamp
  ...
  for (i in 1:length(Symbols)) {
    ...
    for (j in 1:length(str_obs[[1]])) {
      ...
      for (k in 1:ticker_length) {
        vec_row <-
          append(vec_row, values = substr(str_par[[1]][k], 4 + nchar(ticker_header[k]), nchar(str_par[[1]][k])))
      }
    ...
    nts <- xts(
      apply(res[, 2:length(res)], 2, as.numeric),
      as.POSIXct(
...
"getSymbols.Alor" <-
  function #S3 function (Alor is a class of first argument)
(
  Symbols,
  env,
  return.class = 'xts',
  ...
  {
  importDefaults("getSymbols.Alor")
  #rewrite default values if specified by setDefaults
  local_env <- environment()
  for (var in names(list(...))) {
    # import all named elements that are NON formals
  ...
  for (i in 1:length(Symbols)) {
    ...
    repeat {
      ...
      res$Date <-
        paste(res$Date, " ", res$Time) #merge time a date columns
      ...
      Alor.to <-
        as.POSIXct(res$Date[nrow(res)]) + Alor.period * 60 #update a date of observation
      result <-
        if (is.null(result))
          res[order(res$Date),]
      else
        merge(result, res, all = TRUE)
    ...
    newts <-
      xts(apply(result[, 3:length(result)], 2, as.numeric) , as.POSIXct(result[, 1]))
      ...
      if (p == 'Hourly')
      {
        fr <-
          xts(
            ...
            as.POSIXct(strptime(fr[, 1], "%m/%d/%Y %I:%M:%S %p")),
            src = 'rogov',
            updated = Sys.time()
          )
        colnames(fr) <-
          c('Base Value', 'F Value', 'B Value', 'R Value', 'Y Value')
      }
      if (p == 'Monthly')
      {
        fr <-
...
"getSymbols.Finam" <-
           ...
           ...)
  {
    ...
    finam.URL <-
      "/table.csv?d=d&market=1&f=table&e=.csv&dtf=1&tmf=1&MSOR=0&sep=1&sep2=1&at=1&"
    ...
    for (i in 1:length(Symbols)) {
      ...
      from.y <-
        as.numeric(strsplit(as.character(as.Date(from, origin = '1970-01-01')), '-')[[1]][1])
      from.m <-
        as.numeric(strsplit(as.character(as.Date(from, origin = '1970-01-01')), '-')[[1]][2]) - 1
      from.d <-
        as.numeric(strsplit(as.character(as.Date(from, origin = '1970-01-01')), '-')[[1]][3])
      to.y <-
        as.numeric(strsplit(as.character(as.Date(to, origin = '1970-01-01')), '-')[[1]][1])
      to.m <-
        as.numeric(strsplit(as.character(as.Date(to, origin = '1970-01-01')), '-')[[1]][2]) - 1
      to.d <-
        as.numeric(strsplit(as.character(as.Date(to, origin = '1970-01-01')), '-')[[1]][3])
      ...
      if (p == 1) {
        fr <-
          xts(
            apply(as.matrix(fr[, (5:6)]), 2, as.numeric),
            as.POSIXct(strptime(paste(fr[, 3], fr[, 4]), "%Y%m%d %H%M%S")),
            src = 'finam',
            updated = Sys.time()
          )
        ...
        fr <-
          xts(
            apply(as.matrix(fr[, (5:9)]), 2, as.numeric),
...
"loadStockList" <-
  function (verbose = FALSE) {
    ...
    data <-
      data.frame(names[1:min(length(res), length(names))], res[1:min(length(res), length(names))], markets[1:min(length(res), length(names))])
    data[data[, 3] != 3, ] -> data
    rbind(data[(data[, 1] %in% data[data[, 3] == 1, 1]) &
                 data[, 3] == 1, ], data[!(data[, 1] %in% data[data[, 3] == 1, 1]), ]) ->
      data
    data[, 2] -> res
    names(res) <- data[, 1]
    return(res)
        ...
        if ('matrix' %in% return.class) {
          ...
          if ('timeSeries' %in% return.class) {
            if (requireNamespace("timeSeries", quietly = TRUE)) {
              ...
              warning(
                paste(
                  "'timeSeries' from package 'timeSeries' could not be loaded:",
                  " 'xts' class returned"
                )
              )
            }
